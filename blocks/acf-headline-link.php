...
$output = '<div id="' . $id . '" class="' . $block_class . $align_class . ' headline-link background-' . $background_color . '"><div class="wp-block-group__inner-container is-layout-constrained wp-block-group-is-layout-constrained">';
...
if( $title || $description || $links ) {
	...
	if( $link ) {
		$output .= '<p class="call-to-action"><a href="' . $link['link'] . '"';
	...
	$output .= '<span class="hidden show-in-editor">Please add a title, description, or link to this block using the block settings in the sidebar.</span>';
